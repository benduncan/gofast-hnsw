...
func New(m int, mmax int, mmax0 int, efconstruction int, vecsize int) (h HNSW, err error) {
	...
	// Optimal Ml is 1/ln(M) which corresponds to an advantage of the controllable hierarchy for the overlap on different layers to keep it small to reduce the average number of hops in a greedy search on each layer.
